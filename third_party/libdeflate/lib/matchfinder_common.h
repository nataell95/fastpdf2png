	  ...
	  const u32 start_len, const u32 max_len)
{
	...
	if (UNALIGNED_ACCESS_IS_FAST) {
		/*
		 * Fast path: check if the match extends to max_len by
		 * comparing the last word first. For repetitive data
		 * (e.g., white PNG rows), this instantly confirms full-length
		 * matches without the loop.
		 */
		if (likely(max_len >= len + WORDBYTES)) {
			v_word = load_word_unaligned(&matchptr[max_len - WORDBYTES]) ^
				 load_word_unaligned(&strptr[max_len - WORDBYTES]);
			if (v_word == 0) {
				/* Last word matches — likely full match.
				 * Verify from current position. */

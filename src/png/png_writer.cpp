...
struct CompressorDeleter {
    void operator()(libdeflate_compressor* c) const noexcept {
        if (c) libdeflate_free_compressor(c);
    }
};

libdeflate_compressor* GetCompressor() {
  static thread_local std::unique_ptr<libdeflate_compressor, CompressorDeleter> comp(
      libdeflate_alloc_compressor(4));
  return comp.get();
}
...
struct ReusableBuffers {
  ...
  ~ReusableBuffers() { std::free(raw); std::free(comp); }
